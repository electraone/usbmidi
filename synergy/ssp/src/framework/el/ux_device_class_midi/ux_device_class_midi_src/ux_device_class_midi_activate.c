...
UX_SLAVE_ENDPOINT                       *endpoint_interrupt;
    ...
    while (endpoint_interrupt != UX_NULL)
    {
        ...
        if ((endpoint_interrupt -> ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION) == UX_ENDPOINT_IN &&
            (endpoint_interrupt -> ux_slave_endpoint_descriptor.bmAttributes & UX_MASK_ENDPOINT_TYPE) == UX_BULK_ENDPOINT)
        {
    ...
    midi -> ux_device_class_midi_protocol = UX_DEVICE_CLASS_MIDI_PROTOCOL_REPORT;

    /* Save the endpoint in the midi instance.  */
    midi -> ux_device_class_midi_interrupt_endpoint         = endpoint_interrupt;
